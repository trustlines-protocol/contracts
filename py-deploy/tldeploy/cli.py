...
from .core import (
    ...
    remove_owner_of_network, deploy_gnosis_safe, deploy_gnosis_safe_proxy_factory,
)
...
beacon_address_option = click.option(
    "--beacon",
    "beacon_address",
    help="Address of the beacon contract the proxy(ies) will point to",
    required=True,
    type=str,
    callback=validate_address,
)
proxy_owner_option = click.option(
    "--owner",
    "owner_address",
...
def test(
    ...
    gnosis_safe = deploy_gnosis_safe(web3=web3, transaction_options=transaction_options, private_key=private_key)
    gnosis_safe_proxy_factory = deploy_gnosis_safe_proxy_factory(web3=web3, transaction_options=transaction_options,
                                                                 private_key=private_key)
    ...
    click.echo(
        "GnosisSafeL2: {}".format(
            to_checksum_address(gnosis_safe.address)
        )
    )
    click.echo(
...
@cli.command(short_help="Deploy a currency network proxy contract.")
@click.argument("name", type=str)
@click.argument("symbol", type=str)
@click.option(
    "--decimals", help="Number of decimals of the network", default=4, show_default=True
...
@cli.command(
    short_help="Unfreeze a CurrencyNetworkOwnable contract and remove its owner."
)
@click.option(
    "--address",
    "currency_network_address",
    help="Address of the currency network to unfreeze and remove the owner of",
    required=True,
    type=str,
    callback=validate_address,
)
