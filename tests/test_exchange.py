...
def currency_network_contract_with_trustlines(web3, exchange_contract, accounts):
    contract = deploy_network(
        web3, name="TestCoin", symbol="T", decimals=6, fee_divisor=0
    )
...
def test_order_hash(
    ...
    assert (
        order.hash()
        == exchange_contract.functions.getOrderHash(
            [
                order.maker_address,
                order.taker_address,
                order.maker_token,
...
def test_order_signature(
    ...
    assert exchange_contract.functions.isValidSignature(
        maker_address, order.hash().hex(), v, r, s
    ).call()
...
def test_exchange(
    ...
    assert (
        currency_network_contract_with_trustlines.functions.balance(
            maker_address, mediator_address
        ).call()
        == 0
    )
    assert (
        currency_network_contract_with_trustlines.functions.balance(
            mediator_address, taker_address
        ).call()
        == 0
    )

    token_contract.functions.approve(exchange_contract.address, 100).transact(
        {"from": maker_address}
    )
    ...
    assert (
        currency_network_contract_with_trustlines.functions.balance(
            taker_address, mediator_address
        ).call()
        == -50
    )
